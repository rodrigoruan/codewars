...
function isLanguageDiverse(list) {
  const js = list.filter(({ language }) => language === 'JavaScript').length
  const ruby = list.filter(({ language }) => language === 'Ruby').length
  const py = list.filter(({ language }) => language === 'Python').length

  return Math.max(js, ruby, py) / 2 <= Math.min(js, ruby, py)
}
